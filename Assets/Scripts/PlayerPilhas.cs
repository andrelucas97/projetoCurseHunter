...
public class PlayerPilhas : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position, pilha.transform.position) < 1.5f)
        {
            ...
            if (Input.GetKeyDown(KeyCode.E)){
                ...
                if (bateriaAtual > bateriaMaximo){
                    bateriaAtual = bateriaMaximo;
                }
    ...
    {
        mensagemPilhaAdicionado.SetActive(true);
        yield return new WaitForSeconds(duration);
        mensagemPilhaAdicionado.SetActive(false);
    }
